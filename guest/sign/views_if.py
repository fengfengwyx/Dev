...
def get_event_list(request):
    ...
    '''
    如果发布会 id 不为空，优先通过 id 查询，因为 id 的唯一性，查询结果只会有一条，
    将查询结果 以 key:value 对的方式存放到定义的 event 字典中，并将数据字典作为整个返回字典中 data 对应的值返回
    '''
    ...
    '''
    name 查询为模糊查询，查询数据可能会有多条，返回的数据稍显复杂
    首先将查询的每一条数据放到一个字典 event 中，再把每一个字典再放到数组 datas 中，最后再将整个数组做为返回字典中 data 对应的值返回。
    '''
...
def add_guest(request):
    ...
    if n_time >= e_time:
        return JsonResponse({'status': 10025, 'message': '发布会已经开始！'})
...
def user_sign(request):
    ...
    if not result:
        return JsonResponse({'status': 10025, 'message': '手机号码错误！'})
    ...
    if result:
        return JsonResponse({'status': 10027, 'message': '已签到！'})
    else:
